...
class DETRHOI(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries,
        ...
        self.query_embed_verb = MLP(512, hidden_dim , hidden_dim, 2)
    def forward(self, samples: NestedTensor, gt_items=None, verb_embs=None):
        gt_q_mask = None
        #assert len(gt_items) == len(verb_embs)
        if self.aux_loss and gt_items and verb_embs is not None:
            max_len = 0
            bs = len(gt_items)
            ...
            #gt_verb_vec = torch.zeros([max_len, bs, 512]).type_as(gt_items[0])
            gt_q_mask = torch.ones([bs, max_len]).type_as(gt_items[0]).type(torch.bool)
            gt_verb_vec =torch.zeros([max_len, bs, 512]).type_as(verb_embs[0])
            ...
            for i in range(len(gt_items)):
                ...
                #gt_verb_vec[0:len(item), i, :] = item[:, 512+12:512+512+12]
                gt_q_mask[i, 0:len(item)] = False
            for i in range(len(verb_embs)):
                emb = verb_embs[i]
                gt_verb_vec[0:len(emb), i, :] = emb[:, 0:512]
            # todo
            t_sp = torch.tanh(self.query_embed_sp(gt_sp_vec))
            t_obj_emb = self.query_embed_obj(gt_obj_vec)
            t_verb_emb = self.query_embed_verb(gt_verb_vec)
        ##########################################################
        ...
        if self.aux_loss and gt_items is not None:
            ho_att, ho_att_gt, inter_att, inter_att_gt, ho_hs, ho_hs_gt, inter_hs, inter_hs_gt = self.transformer(self.input_proj(src), mask, self.query_embed.weight,
                                                      query_embed_q=t_sp, query_embed_e=t_obj_emb,
                                                      query_embed_v=t_verb_emb,
                                                      query_embed2_mask=gt_q_mask,
                                                      pos_embed=pos[-1])

            outputs_obj_class = self.obj_class_embed(ho_hs)
            outputs_verb_class = self.verb_class_embed(inter_hs)
            outputs_sub_coord = self.sub_bbox_embed(ho_hs).sigmoid()
            outputs_obj_coord = self.obj_bbox_embed(ho_hs).sigmoid()

            outputs_gt_obj_class = self.obj_class_embed(ho_hs_gt)
            outputs_gt_verb_class = self.verb_class_embed(inter_hs_gt)
            outputs_gt_sub_coord = self.sub_bbox_embed(ho_hs_gt).sigmoid()
            outputs_gt_obj_coord = self.obj_bbox_embed(ho_hs_gt).sigmoid()
            out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
                   ...
                   'ho_hs': ho_hs[-1], 'ho_hs_gt': ho_hs_gt[-1],
                   'inter_hs': inter_hs[-1], 'inter_hs_gt': inter_hs_gt[-1],
                   'ho_att': ho_att[-1], 'ho_att_gt': ho_att_gt[-1],
                   'inter_att': inter_att[-1], 'inter_att_gt': inter_att_gt[-1]
            }
            out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                    ...
                                                    ho_hs, ho_hs_gt, inter_hs, inter_hs_gt,
                                                    ho_att, ho_att_gt, inter_att, inter_att_gt)
            '''
            out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
                   'pred_sub_boxes': outputs_sub_coord[-1], 'pred_obj_boxes': outputs_obj_coord[-1],
                   'gt_obj_logits': outputs_gt_obj_class[-1], 'gt_verb_logits': outputs_gt_verb_class[-1],
            ...
            ho_hs, inter_hs = self.transformer(self.input_proj(src), mask, self.query_embed.weight,
                                  pos_embed=pos[-1])
            outputs_obj_class = self.obj_class_embed(ho_hs)
            outputs_verb_class = self.verb_class_embed(inter_hs)
            outputs_sub_coord = self.sub_bbox_embed(ho_hs).sigmoid()
    ...
    def _set_aux_loss(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord,
                      ...
                      ho_hs, ho_hs_gt, inter_hs, inter_hs_gt, ho_att, ho_att_gt, inter_att, inter_att_gt):
        return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d,
                 ...
                 'ho_hs': ho_h1, 'ho_hs_gt': ho_h2, 'inter_hs': inter_h1, 'inter_hs_gt': inter_h2,
                 'ho_att': ho_att1, 'ho_att_gt': ho_att2, 'inter_att': inter_att1, 'inter_att_gt': inter_att2}
                for a, b, c, d, a1, b1, c1, d1, ho_h1, ho_h2, inter_h1, inter_h2, ho_att1, ho_att2, inter_att1, inter_att2 in
                zip(outputs_obj_class[:-1], outputs_verb_class[:-1],
                    ...
                    ho_hs[:-1], ho_hs_gt[:-1], inter_hs[:-1], inter_hs_gt[:-1],
                    ho_att[:-1], ho_att_gt[:-1], inter_att[:-1], inter_att_gt[:-1]
                    )]
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_verb_cos_between_ts(self, outputs, targets, indices, indices_gt, num_interactions):
        assert 'pred_verb_logits' in outputs
        stu_logits = outputs['pred_verb_logits']
        tea_logits = outputs['gt_verb_logits']
        idx = self._get_src_permutation_idx(indices)
        target_hs = stu_logits[idx]
        target_hs = F.softmax(target_hs,dim =-1)
        target_hs_gt = torch.cat([t[i] for t, (_, i) in zip(tea_logits, indices)], dim=0)
        target_hs_gt = F.softmax(target_hs_gt,dim =-1)
        if len(target_hs) > 0:
            #loss_match = nn.CosineEmbeddingLoss()(target_hs, target_hs_gt,
                                                  #torch.ones([target_hs.size()[0]]).to(target_hs_gt.device))
            loss_match = 10 * nn.KLDivLoss()((target_hs + 1e-6).log(), target_hs_gt) / 3
        else:
            loss_match = torch.zeros([]).type_as(target_hs)
        losses = {'loss_verb_pred_match': loss_match}
        return losses
    ...
    def loss_match_cosine_ho(self, outputs, targets, indices, indices_gt, num_interactions):
        hs = outputs['ho_hs']
        hs_gt = outputs['ho_hs_gt']
        ...
        if len(target_hs) > 0:
            # todo
            # pred_verb_logits = outputs['pred_verb_logits'].sigmoid()
            # pred_verb_logits_t = outputs['gt_verb_logits'].sigmoid()
            # pred_verb_logits = pred_verb_logits[idx]
            # pred_verb_logits_t = torch.cat([t[i] for t, (_, i) in zip(pred_verb_logits_t, indices)], dim=0)
            # weight = torch.mean(abs(pred_verb_logits_t - pred_verb_logits), dim=-1)
            # loss_match = nn.CosineEmbeddingLoss(reduction='none')(target_hs, target_hs_gt,
            #                                                       torch.ones([target_hs.size()[0]]).to(
            #                                                           target_hs_gt.device)) * (1+weight)
            #
            # loss_match = torch.mean(loss_match)

            loss_match = nn.CosineEmbeddingLoss()(target_hs, target_hs_gt,
                                                  torch.ones([target_hs.size()[0]]).to(target_hs_gt.device))
        else:
            loss_match = torch.zeros([]).type_as(target_hs)
        losses = {'loss_match_ho': loss_match}
        return losses

    def loss_match_cosine_inter(self, outputs, targets, indices, indices_gt, num_interactions):
        hs = outputs['inter_hs']
        hs_gt = outputs['inter_hs_gt']
        ...
        if len(target_hs) > 0:
            ...
            loss_match = torch.zeros([]).type_as(target_hs)
        losses = {'loss_match_inter': loss_match}
        return losses

    def loss_match_kl_ho(self, outputs, targets, indices, indices_gt, num_interactions):
        hs = outputs['ho_att']
        hs_gt = outputs['ho_att_gt']
        # idx = self._get_src_permutation_idx(indices)
        ...
        if len(target_hs) > 0:
            ...
            loss_match = torch.zeros([]).type_as(target_hs)
        losses = {'loss_match_att_ho': loss_match}
        return losses

    def loss_match_kl_inter(self, outputs, targets, indices, indices_gt, num_interactions):
        hs = outputs['inter_att']
        hs_gt = outputs['inter_att_gt']
        # idx = self._get_src_permutation_idx(indices)
        # target_hs = hs[idx[0], :, idx[0], :]
        # target_hs_gt = torch.cat([t[:, i, :] for t, (_, i) in zip(hs_gt, indices)], dim=1)
        #
        ...
        if len(target_hs) > 0:
            ...
            loss_match = torch.zeros([]).type_as(target_hs)
        losses = {'loss_match_att_inter': loss_match}
        return losses
    ...
    def _neg_loss(self, pred, gt):
        ...
        if not math.isfinite(loss):
            #file_path = 'outputs/los_neg_loss.txt'
    ...
    def get_loss(self, loss, outputs, targets, indices, indices_gt, num, **kwargs):
        loss_map = {
            ...
            'ho_match': self.loss_match_cosine_ho,
            'inter_match': self.loss_match_cosine_inter,
            'ho_match_att': self.loss_match_kl_ho,
            'inter_match_att': self.loss_match_kl_inter,
            "verb_pred_match" : self.loss_verb_cos_between_ts
        }
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    if loss == "ho_match" or (loss == "inter_match") or (loss == "ho_match_att" and i < 3) or (loss == "inter_match_att" and i < 3) or (loss =="verb_pred_match"):
                        continue
...
def build(args):
    ...
    weight_dict['loss_match_ho'] = args.ho_match_coef
    weight_dict['loss_match_att_ho'] = args.ho_match_att_coef
    weight_dict['loss_match_inter'] = args.inter_match_coef
    weight_dict['loss_match_att_inter'] = args.inter_match_att_coef
    weight_dict['loss_verb_pred_match'] = args.verb_pred_match
    ...
    losses = ['obj_labels', 'verb_labels', 'sub_obj_boxes', 'obj_cardinality',
              'gt_obj_labels', 'gt_verb_labels', 'gt_sub_obj_boxes', 'ho_match', 'inter_match','ho_match_att', 'inter_match_att','verb_pred_match']
