...
class CDN_TS(nn.Module):

    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                 num_dec_layers_hopd=6, num_dec_layers_interaction=6,
        ...
        decoder_layer = TransformerDecoderLayerAttMap(d_model, nhead, dim_feedforward,
                                                      dropout, activation, normalize_before)
        decoder_norm = nn.LayerNorm(d_model)
        self.decoder = TransformerDecoder_ts(decoder_layer, num_dec_layers_hopd, decoder_norm,
                                          return_intermediate=return_intermediate_dec)
        interaction_decoder_layer = TransformerDecoderLayerAttMap(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before)
        interaction_decoder_norm = nn.LayerNorm(d_model)
        self.interaction_decoder = TransformerDecoder_ts(interaction_decoder_layer, num_dec_layers_interaction, interaction_decoder_norm,
                                            return_intermediate=return_intermediate_dec)
    ...
    def forward(self, src, mask, query_embed1,
                query_embed_q=None, query_embed_e=None,query_embed_v=None,
        ...
        if query_embed_q is not None:
            ho_att_s, ho_att_t, ho_hs_s, ho_hs_t = self.decoder.forwardt(tgt1, memory,
            ...
            ho_hs_s = ho_hs_s.transpose(1, 2)
            ho_hs_t = ho_hs_t.transpose(1, 2)
            interaction_query_pos_s = ho_hs_s[-1].permute(1, 0, 2)
            interaction_query_pos_t = ho_hs_t[-1].permute(1, 0, 2)
            tgt2 = torch.zeros_like(interaction_query_pos_s)
            inter_att_s, inter_att_t, inter_hs_s, inter_hs_t =self.interaction_decoder.forwardt(tgt2,memory,
                                                         memory_key_padding_mask=mask,
                                                        tgt_key_padding_mask_t=query_embed2_mask,
                                                        pos=pos_embed,
                                                        query_pos=interaction_query_pos_s,
                                                        query_embed_q=interaction_query_pos_t,
            ...
            ho_hs = self.decoder(tgt1, memory,
                              memory_key_padding_mask = mask,
                              pos = pos_embed,
                              query_pos = query_embed1)
            ho_hs = ho_hs.transpose(1, 2)
            interaction_query = ho_hs[-1].permute(1, 0, 2)
            tgt2 = torch.zeros_like(interaction_query)
            inter_hs = self.interaction_decoder(tgt2, memory,
                            memory_key_padding_mask = mask,
                            pos = pos_embed,
                            query_pos = interaction_query)
            return ho_hs, inter_hs.transpose(1, 2)
'''
...
class TransformerDecoder_ts(nn.Module):
    ...
    def forwardt(self, tgt, memory,
        ...
        return torch.stack(atts_all), torch.stack(attt_all), torch.stack(hs), torch.stack(hs_t)
...
def build_hoi_ts(args):
    return CDN_TS(
        ...
        num_encoder_layers=args.enc_layers,
        #num_decoder_layers=args.dec_layers,
        num_dec_layers_hopd=6,
        num_dec_layers_interaction=6,
        normalize_before=args.pre_norm,
        return_intermediate_dec=True,
    )
