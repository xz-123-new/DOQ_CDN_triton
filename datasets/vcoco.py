...
class VCOCO(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries, dataset_file):
        ...
        self.word2vec_verb = np.load("data/v-coco/annotations/vcoco_clipvec_verb.npy")
        ...
        if self.img_set == 'train':
            ...
            for hoi in img_anno['hoi_annotation']:
                ...
                if sub_obj_pair in sub_obj_pairs:
                    ...
                    verb_embs[sub_obj_pairs.index(sub_obj_pair)] = verb_embs[sub_obj_pairs.index(sub_obj_pair)] + \
                                                self.word2vec_verb[self._valid_verb_ids.index(hoi['category_id'])]
                else:
                    ...
                    verb_emb = self.word2vec_verb[self._valid_verb_ids.index(hoi['category_id'])]
            ...
            if len(sub_obj_pairs) == 0:
                ...
                target['verb_embs'] = torch.zeros((0, 512), dtype=torch.float32)
            else:
                ...
                target['verb_embs'] = torch.as_tensor(verb_embs, dtype=torch.float32)
        else:
            target['boxes'] = boxes
            target['labels'] = classes
            target['id'] = idx
