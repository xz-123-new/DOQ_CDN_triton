...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, max_norm: float = 0, model_name="baseline"):
    model.train()
    criterion.train()
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if model_name == "baseline":
            ...
            verb_embs = [t["verb_embs"] for t in targets]
            #pdb.set_trace()
            outputs = model(samples, gt_items,verb_embs)
        ...
        metric_logger.update(lr_new=optimizer.param_groups[0]["lr"])
